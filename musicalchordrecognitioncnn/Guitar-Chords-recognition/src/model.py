...
class CNN(object):
    ...
    def load_model(self):
        ...
        try:
            ...
            logger.error("Model not found. Error: ", e)

    def predict(self, filepath, loadmodel=True):
        """Predicts the chord from an audio file."""
        logger.info('Prediction')
        try:
            if loadmodel:
                # self.load_model()  # Uncomment and implement this if needed
                pass

            y, sr = librosa.load(filepath, duration=2)
            ps = librosa.feature.melspectrogram(y=y, sr=sr)
            ps = np.array(ps.reshape((1,) + self.input_shape))

            predictions = self.model.predict(ps)
            chord_index = np.argmax(predictions)

            if chord_index < len(CLASSES):
                chord = CLASSES[chord_index]
                logger.info("The recorded chord is " + chord)
            else:
                chord = "N/A"
        except Exception as e:
            logger.error("Error occurred during prediction: ", e)
            chord = "N/A"
        return chord
